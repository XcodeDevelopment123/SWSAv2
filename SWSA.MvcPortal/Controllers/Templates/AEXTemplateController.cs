...
{
    private readonly DbSet<AEXTemplate> _aexTemplates = db.Set<AEXTemplate>();

    [Route("")]
    public async Task<IActionResult> AEXTemplate()
    {
        var data = await _aexTemplates
            .Include(c => c.Client)
    ...
    {
        var task = await _aexTemplates.Where(c => c.Id == id)
            .Include(c => c.PersonInCharge)
            .Include(c => c.Client)
            .FirstOrDefaultAsync();
        return Ok(task);
    ...
    [InternalAjaxOnly]
    [HttpPost("create")]
    public async Task<IActionResult> Create(AEXTemplate req)
    {
        _aexTemplates.Add(req);
    ...
    public async Task<IActionResult> Update([FromRoute] int id, AEXTemplate req)
    {
        var task = await _aexTemplates.FirstOrDefaultAsync(c => c.Id == id);
        if (task == null)
        {
            return Ok();
        }
        ...
        task.SecondReviewFinal = req.SecondReviewFinal;
    ...
    {
        var task = await _aexTemplates.FindAsync(id);
        if (task == null)
        {
            return NotFound();
        }
        _aexTemplates.Remove(task);
        await db.SaveChangesAsync();
