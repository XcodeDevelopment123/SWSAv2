$(function () {
    ...
    }).buttons().container().appendTo('#workDatatable_wrapper .col-md-6:eq(0)');
    ...
    $(document).on('click', '.btn-delete', function () {
        ...
        if (confirm(`Are you sure you want to delete "${name}"?`)) {
            $.ajax({
                url: `${urls.company_works}/${id}/delete`,
                method: "DELETE",
                success: function (res) {
                    if (res) {
                        Toast_Fire(ICON_SUCCESS, "Deleted", "Work deleted successfully.");
